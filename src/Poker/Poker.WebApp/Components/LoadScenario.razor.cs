...
namespace Poker.WebApp.Components;
...
public partial class LoadScenario
{
    ...
    [Parameter]
    public Dictionary<string, string> FixedValue { get; set; }

    [Parameter]
    public Dictionary<string, string> FixedSuit { get; set; }

    [Parameter]
    public Dictionary<int, string> FixedPosition { get; set; }
    ...
    {
        RNGCryptoServiceProvider provider = new RNGCryptoServiceProvider(); // RandomNumberGenerator
        int n = list.Count;
        while (n > 1)
        {
            byte[] box = new byte[1];
